...
3. 실행이 막히면 — 미서명 배포판이라 OS마다 한 번씩 걸립니다.
   ...
   - **macOS**: Gatekeeper가 "'installer'을(를) 열지 않음"이라며 **아예 막습니다**. 화면 캡처로 따라가는
     ...
     설치 도중 본체(`inno-creed`)에 대해 같은 경고가 한 번 더 뜨던 것은 **없어졌습니다** — 인스톨러가 설치한
     파일의 격리 표시를 스스로 뗍니다(v2.3.1~). 옛 인스톨러로 그 경고를 만났다면 같은 절차를 한 번 더 거치면 됩니다.
...
**v2.3.1부터** 같은 ZIP에 두 실행 파일이 들어갑니다. 그보다 옛 릴리즈를 쓰고 있어 `installer-cli`가 없다면 아래의 기존 실행 방법을 사용하세요.
...
설정 파일 후보를 확인하고, 다른 파일을 쓰려면 전체 경로를 입력합니다. 설치 위치는 Enter로 기본값을 사용하거나, 다른 상위 폴더를 입력하면 그 안의 `inno-creed` 폴더를 사용합니다. **설치 확인은 `[Y/n]`이라 Enter가 곧 설치입니다**(취소하려면 `n`). 반대로 `--uninstall` 제거 확인은 `[y/N]`이라 Enter가 취소입니다. Claude Desktop이 켜져 있으면 종료 후 다시 확인합니다. 확장 프로그램 연결 안내(전 OS)와 선택적 `doctor` 인증 진단도 제공됩니다. Ctrl+C로 중단할 수 있습니다.
...
크레덴셜을 어느 소스에서 잡았는지(또는 어디서 막혔는지), 익스텐션 브릿지·크레덴셜 파일·Claude Desktop 설정 파일의 실제 위치, 그리고 **gw에 요청을 보내 실제로 인증이 되는지**(1회, 401이면 재취득 후 1회 더)까지 한 화면에 보여줍니다. 토큰 값은 출력하지 않으므로 그대로 캡처해 공유해도 됩니다.
...
- **Chrome/Edge는 있는데 "복호화 실패"라고 나옴** →
  ...
  - **Windows Chrome/Edge app-bound(`v20`)**: 호출자 프로세스 경로를 검증하므로 inno-creed 같은 제3자 프로세스로는 **설계상 항상 거부**됩니다(버전·설정과 무관 — inno-creed는 `v20` 표식을 보면 복호화를 시도하지 않고 그 자리에서 사유를 알려줍니다). 확장 프로그램(4번)을 쓰세요.
...
| 브라우저(Chrome/Edge/Firefox) | 브라우저로 다시 로그인 | **불필요** — 401을 만나면 그 호출 안에서 쿠키를 다시 읽고 한 번 재전송합니다 |
