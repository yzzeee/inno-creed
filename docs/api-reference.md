...
`send_mail`과 `save_mail_draft`는 필수 `body`에 일반 텍스트 또는 Markdown을 받는다.
**이전 `html` 인자는 지원하지 않는다.** HTML을 Markdown으로 조용히 재해석하지 않고 오류로
반환하므로, MCP를 재연결해 갱신된 스키마를 읽고 `body`로 호출해야 한다. 알 수 없는 인자,
본문 누락·공백·표시할 텍스트가 없는 본문·직접 HTML 태그는 세션/업로드/발송 전에 거부한다.
HTML 예시를 본문에 싣는 경우 Markdown 코드 블록 또는 인라인 코드를 사용한다.

- HTML 변환(문단·줄바꿈·목록·표)과 등록 서명 삽입은 도구가 담당한다.
- `save_mail_draft`: A14 저장 후 A01(draft)로 HTML을 다시 읽어 작성 HTML과 **텍스트를
  공백 정규화 후 전체 비교**한다. 서명만 남거나 일부 문단이 빠지면 오류다.
  이는 시각적 렌더링·링크 목적지·이미지 보존 전체를 보장하는 검사는 아니다.
- 검증 실패/조회 실패 시 **발송하지 않는다**. 오류에 `draft_muid`를 남긴다. 초안 저장은
  이미 됐으므로 다시 저장하기 전에 해당 초안을 확인한다.
- 검증 성공 시 서버에서 읽은 HTML의 SHA-256을 설정 디렉토리의
  `verified-mail-bodies/`에 계정·초안별로 기록한다. 본문 원문은 파일에 저장하지 않는다.
- `send_mail_from_draft`: 발송 직전 다시 연 HTML이 기록의 해시와 일치해야 한다.
  기록 없음·읽기 실패·본문 변경은 발송하지 않는다. **웹/구버전 초안은 웹에서 발송하거나
  `save_mail_draft(body)`로 새로 작성**해야 한다. 다른 머신에는 검증 기록이 자동 이전되지 않는다.
  서식만 바뀌어도 해시는 달라져 재작성이 필요하다. 검증은 본문 대상이며 제목·수신자·첨부의
  변경 이력까지 고정하는 기능은 아니다. 발송 성공 후 기록을 삭제한다.
- `send_mail`도 내부적으로 **초안 저장→본문 검증→해당 초안 발송**을 실행한다.
  Agent가 발송 단계에서 HTML을 다시 조립하지 않는다. API 성공 이후 수신 확인은 별도다.
  최종 발송 API 응답을 받지 못하면 "발송 여부 미확정" 오류다. 자동 재발송하지 말고
  보낸메일함을 확인한다. 발송 전 검증 실패의 "발송하지 않았습니다"와 구분한다.
